...
resource "aws_s3_bucket" "S3BucketForConfig" {
  bucket = "s3-bucket-random-name-GwezN"
}
    ...
    "Statement": [
        {
            ...
            "Resource": [
                "arn:aws:s3:::s3-bucket-random-name-GwezN/*"
            ],
            "Condition": {
                "StringLike": {
        ...
        },
        {
            ...
            "Resource": "arn:aws:s3:::s3-bucket-random-name-GwezN"
        }
    ]
}
POLICY
...
module "SnsTopic1" {
  ...
  stack_name = "tf-cfn-stack-SnsTopic1-aZKtY"
}
