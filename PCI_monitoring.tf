...
resource "aws_s3_bucket" "S3BucketForConfig" {
  bucket = "s3-bucket-random-name-iHyyk"
}
    ...
    "Statement": [
        {
            ...
            "Resource": [
                "arn:aws:s3:::s3-bucket-random-name-iHyyk/*"
            ],
            "Condition": {
                "StringLike": {
        ...
        },
        {
            ...
            "Resource": "arn:aws:s3:::s3-bucket-random-name-iHyyk"
        }
    ]
}
POLICY
...
module "SnsTopic1" {
  ...
  stack_name = "tf-cfn-stack-SnsTopic1-tdzTK"
}
